  ...
  SetPriceAndBonusPoints = lambda { |rentals, customer|
    ...
    rentals.each do |rental|
      ...
      if rental.movie.price_code == Movie::NEW_RELEASE
        ...
        rental.days_rented > 2 ? this_amount += (rental.days_rented - 2) * 1.5 : this_amount += 2
      ...
      rental.days_rented > 3 ? this_amount += (rental.days_rented - 3) * 1.5 : this_amount += 1.5
      frequent_renter_points += 1
      end
      result += "\t" + rental.movie.title_for_movie + "\t" + "$" + this_amount.to_s + "\n"
      amount_owed += this_amount
    end
